...
def process_runs_for_base(
    ...
    """
    run_metrics = []
    run_pattern = re.compile(r'_run_(\d+)\.json$', re.IGNORECASE)
    for jf in run_files:
        res = evaluate_json_file(jf)
        # --- Run-Index extrahieren ---
        m = run_pattern.search(jf)
        run_idx = int(m.group(1)) if m else 0
        run_metrics.append({"run_idx": run_idx, **res, "file_path": jf})
        
    
    run_metrics.sort(key=lambda x: x["run_idx"])
...
def write_excel_summary(
    ...
    header = [
        ...
        "correct_run1", "correct_run2", "correct_run3", "correct_run4", "correct_run5",
        "incorrect_run1", "incorrect_run2", "incorrect_run3", "incorrect_run4", "incorrect_run5",
        "unsure_run1", "unsure_run2", "unsure_run3", "unsure_run4", "unsure_run5"
    ]
    ws.append(header)
...
def write_csv_summary(
    ...
    """
    Schreibt die Zusammenfassung zusätzlich als CSV-Datei.
    Gleiche Struktur wie Excel, aber einfacher CSV-Format.
    """
    header = [
        "Research Question", "Model", "Markers",
        "Accuracy_Mean", "Accuracy_Std",
    ...
    sorted_rows = sorted(results_summary, key=sort_key)
    ...
    with open(output_csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for item in sorted_rows:
            ...
            row = [
                ...
                agg.get("accuracy_lr_mean", ""),
                agg.get("accuracy_lr_std", ""),
                agg.get("f1_lr_mean", ""),
                agg.get("f1_lr_std", ""),

                agg.get("accuracy_ab_mean", ""),
                agg.get("accuracy_ab_std", ""),
                agg.get("f1_ab_mean", ""),
                agg.get("f1_ab_std", ""),
            ...
            for i in range(N_RUNS):
                row.append(run_metrics[i]["correct_count"] if i < run_count else "")
            for i in range(N_RUNS):
                row.append(run_metrics[i]["incorrect_count"] if i < run_count else "")
            for i in range(N_RUNS):
                row.append(run_metrics[i]["unsure_count"] if i < run_count else "")

            writer.writerow(row)

    print(f"CSV summary saved at: {output_csv_path}")
...
def main():
    ...
    base_path = os.path.join(os.environ["WS_MODEL"],
                               "pixtral-12b-finetuned", "results")
    
    unsure_base = os.path.join(os.environ["WS_MODEL"],
                               "pixtral-12b-finetuned", "unsure_cases")
    ...
    excel_path = os.path.join(base_path, "Results_Images.xlsx")
    write_excel_summary(excel_summary_rows, excel_path)

    csv_path = os.path.join(base_path, "Results_Images.csv")
    write_csv_summary(excel_summary_rows, csv_path)
